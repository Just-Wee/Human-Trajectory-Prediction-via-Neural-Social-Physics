    ...
    for i in range(F2.shape[0]):
        ...
        if area[i, 0, 0] == area[i, 0, 1] and area[i, 1, 0] != area[i, 1, 1]:
            environment_vision = semantic_map[
                area[i, 0, 0],
                torch.min(area[i, 1, 0], area[i, 1, 1]) : torch.max(
                    area[i, 1, 0], area[i, 1, 1]
                ),
            ]
            if len(np.argwhere(environment_vision == 5)) == 0:
                continue
            obstacle = torch.from_numpy(
                np.mean(np.argwhere(environment_vision == 5), axis=0)
            ).to(
                device
            )  # 1
        ...
        if area[i, 0, 0] != area[i, 0, 1] and area[i, 1, 0] == area[i, 1, 1]:
            environment_vision = semantic_map[
                torch.min(area[i, 0, 0], area[i, 0, 1]) : torch.max(
                    area[i, 0, 0], area[i, 0, 1]
                ),
                area[i, 1, 0],
            ]
            if len(np.argwhere(environment_vision == 5)) == 0:
        ...
        if area[i, 0, 0] < area[i, 0, 1] and area[i, 1, 0] > area[i, 1, 1]:
            ...
            if dis == 0:
                continue
            F2[i, :] = (k_env / dis) * (
                (torch.tensor([0, k_scope]).to(device) - obstacle) / dis
            )
        if area[i, 0, 0] > area[i, 0, 1] and area[i, 1, 0] < area[i, 1, 1]:
            ...
            if dis == 0:
                continue
            F2[i, :] = (k_env / dis) * (
                (torch.tensor([k_scope, 0]).to(device) - obstacle) / dis
            )
        if area[i, 0, 0] > area[i, 0, 1] and area[i, 1, 0] > area[i, 1, 1]:
            ...
            if dis == 0:
                continue
            F2[i, :] = (k_env / dis) * (
                (torch.tensor([k_scope, k_scope]).to(device) - obstacle) / dis
            )
    ...
    for i in range(F2.shape[0]):
        ...
        if area[i, 0, 0] == area[i, 0, 1] and area[i, 1, 0] != area[i, 1, 1]:
            ...
            if len(np.argwhere(environment_vision == 3)) == 0:
                continue
            obstacle = torch.from_numpy(
                np.mean(np.argwhere(environment_vision == 3), axis=0)
            ).to(
                device
            )  # 1
            if area[i, 1, 0] < area[i, 1, 1]:
    ...
    for i in range(F2.shape[0]):
        ...
        if area[i, 0, 0] == area[i, 0, 1] and area[i, 1, 0] != area[i, 1, 1]:
            ...
            if len(np.argwhere(environment_vision == 4)) == 0:
                continue
            obstacle = torch.from_numpy(
                np.mean(np.argwhere(environment_vision == 4), axis=0)
            ).to(
                device
            )  # 1
            if area[i, 1, 0] < area[i, 1, 1]:
...
class NSP(nn.Module):
    def __init__(
        self, input_size, embedding_size, rnn_size, output_size, enc_size, dec_size
    ):
        '''
        Args:
        ...
        self.encoder_dest_state = MLP(
            input_dim=2, output_dim=output_size, hidden_size=enc_size
        )
        self.dec_tau = MLP(
            input_dim=2 * output_size, output_dim=1, hidden_size=dec_size
        )
        ...
        self.encoder_people_state = MLP(
            input_dim=4, output_dim=output_size, hidden_size=enc_size
        )
        self.dec_para_people = MLP(
            input_dim=2 * output_size, output_dim=1, hidden_size=dec_size
        )
    ...
    def forward_lstm(
        self, input_lstm, hidden_states1, cell_states1, hidden_states2, cell_states2
    ):
        # input_lstm: peds*4

        # LSTM1
        input_embedded1 = self.relu(
            self.input_embedding_layer1(input_lstm)
        )  # peds*embedding_size
        h_nodes1, c_nodes1 = self.cell1(
            input_embedded1, (hidden_states1, cell_states1)
        )  # h_nodes/c_nodes: peds*rnn_size
        outputs1 = self.output_layer1(h_nodes1)  # peds*output_size

        # LSTM2
        input_embedded2 = self.relu(
            self.input_embedding_layer2(input_lstm)
        )  # peds*embedding_size
        h_nodes2, c_nodes2 = self.cell2(
            input_embedded2, (hidden_states2, cell_states2)
        )  # h_nodes/c_nodes: peds*rnn_size
    ...
    def forward_coefficient_people(
        self, outputs_features2, supplement, current_step, current_vel, device
    ):
        ...
        for i in range(num_peds):
            ...
            cosangle = torch.matmul(person_dir, current_vel[i, :]) / (
                dis * torch.norm(current_vel[i, :])
            )  # peds
        ...
        encoding_part1 = torch.unsqueeze(outputs_features2, dim=1).repeat(
            1, self.max_peds, 1
        )  # peds*25*16
        features_others = self.encoder_people_state(
            curr_supp[:, :-1, :-1]
        )  # peds*25*16
        input_coefficients = torch.cat(
            (encoding_part1, features_others), dim=-1
        )  # peds*25*32
        coefficients = torch.squeeze(
            100 * self.sigmoid(self.dec_para_people(input_coefficients))
        )  # peds*25
    ...
    def forward_coefficient_test(
        ...
        curr_state = torch.cat(
            (current_step, current_vel, torch.ones((num_peds, 1)).to(device)), dim=1
        )
        for i in range(num_peds):
            first_part = all_first_part[i]
            peds_con1 = curr_state[first_part, :]  # peds*5
            peds_con1[:, :2] = (
                peds_con1[:, :2] + first_frame[first_part, :] - first_frame[i, :]
            )  # peds*2
            peds_con2 = supplement[
                i, int(supplement[i, -1, 0]) : int(supplement[i, -1, 1]), :
            ]  # peds*5
